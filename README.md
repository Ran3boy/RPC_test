# Нагрузочное тестирование и сравнение REST и gRPC с использованием Locust
...
- **Приложение 2 (gRPC):** Сервис, реализующий взаимодействие по протоколу gRPC с использованием protobuf для сериализации данных.
...
- **Сеть:** 100 Мб/сек Ethernet, тестирование через loopback (127.0.0.1)
...
#### Результаты теста Sanity Check
**Метрики:**
- **REST:** RPS = 4.00, Avg = 6.59 мс, p95 = 8 мс, p99 = 10 мс, Ошибки = 0
...
#### Результаты теста Normal Load
**Метрики:**
- **REST:** RPS = 38.94, Avg = 6.49 мс, p95 = 9 мс, p99 = 12 мс, Ошибки = 0
...
#### Результаты теста Stress Test
**Метрики:**
- **REST:** RPS = 156.59, Avg = 7.04 мс, p95 = 11 мс, p99 = 19 мс, Ошибки = 0
...
#### Результаты теста Stability Test
**Метрики:**
- **REST:** RPS = 23.61, Avg = 6.66 мс, p95 = 9 мс, p99 = 12 мс, Ошибки = 0
...
### 5.2 Сравнительный анализ REST и gRPC
**Численное сравнение:**
...
- Тестирование в распределённой среде с сетевыми задержками 10-50 мс
- Использование Redis/Memcached для кэширования
- Тестирование с реальной СУБД (PostgreSQL) вместо JSON-файла
- Сравнение с дополнительными протоколами (GraphQL, WebSocket)

### 6.4 Ограничения
- Тестирование проводилось на локальной машине, что минимизировало сетевые задержки
- Использование файлового хранилища не отражает типичные production-сценарии
- Модель нагрузки не учитывала географическое распределение пользователей
- Не тестировались операции записи (POST/PUT), которые могут вести себя иначе
...
- **Jin et al. (University of Washington)** — GraphQL vs REST для serverless data persistence
...
- **Niswar et al. (2024)** — REST vs GraphQL vs gRPC в микросервисной архитектуре
  ...
  - Вывод:
    - gRPC — наименьшее время ответа
    - REST — промежуточный результат
    - GraphQL — самое большое время ответа и высокая загрузка CPU
